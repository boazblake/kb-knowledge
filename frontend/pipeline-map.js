...
document.querySelector('.evidence-note p').textContent = `Remediation ${S.commits.remediation}; ${S.remediationStage}; ${S.evidenceScope}. Prior test run ${E.runDate}: Nix full ${E.nixFull.passed}/${E.nixFull.failed}; disposable PostgreSQL ${E.disposablePostgresqlFull.passed}/${E.disposablePostgresqlFull.failed}; targeted P4 ${E.targetedP4.passed}/${E.targetedP4.failed}; labeled scenarios ${E.labels.passed}/${E.labels.total}. Production qualification=false.`;
const stages = [
