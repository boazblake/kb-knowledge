const S = window.VISUALIZATION_STATUS || {horizons:['Prototype/reference','Production-shaped non-prod','Production qualification/release']};
document.querySelector('.no-go').textContent = 'Production NO-GO · GREEN local evidence: PostgreSQL 21/21; custom checks 5/5; migrations 001-005 idempotent; zero live calls.';
document.querySelector('.evidence-note').innerHTML = `<h2>Current facts</h2><ul><li>Local evidence GREEN: authority/purge/recovery 21/21; custom checks 5/5; migrations 001-005 idempotent.</li><li>Remediation validated: batch atomicity, cursor comparator, staged raw/orphan lifecycle, outbox ownership/retry, canonical purge namespace.</li><li>Missing live endpoints: KMS, S3, OIDC, Temporal, OTel; external connector absent.</li><li>Production red: load/SLO, RPO/RTO, recovery, SEC, OPS.</li><li><strong>Exactly one next roadmap item:</strong> ${S.next}</li></ul>`;
const items = [
  ['Sources / connectors','Local files','Prototype/reference','DONE','Local fixtures; frontend browser verification.'],
  ['Sources / connectors','Disposable provider environment','Production-shaped non-prod','BLOCKED','No endpoint or credentials configured; zero live calls.'],
