...
document.querySelector('.no-go').textContent = `${S.decision} · HEAD ${S.commits.head}; tested ancestor ${S.commits.tested}. ${S.evidenceScope}`;
document.querySelector('.evidence-note').innerHTML = `<h2>Current facts</h2><ul><li>${S.evidenceRun.nix.label}: ${visualizationRunLabel(E.nix)}.</li><li>${E.disposablePostgresql.label}: ${visualizationRunLabel(E.disposablePostgresql)}.</li><li>${E.frontendBdd.label}: ${visualizationRunLabel(E.frontendBdd)}; BDD ${S.statuses.bdd}.</li><li>Migrations ${S.migrations.freshRun}; second run ${S.migrations.secondRun}.</li><li>${S.dirtyWorktree}</li><li>Developer ${S.statuses.sre}; test-operator ${S.statuses.qa}; production ${S.statuses.production}.</li><li><strong>Next:</strong> ${S.next}</li></ul>`;
const items = [
  ...
  ['Storage / security','PostgreSQL authority','Production-shaped non-prod','DONE',`${visualizationRunLabel(E.disposablePostgresql)}.`],
