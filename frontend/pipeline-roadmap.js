const S = window.VISUALIZATION_STATUS || {snapshot:'30283d3',suite:{pass:12,run:12},horizons:['Prototype/reference','Production-shaped non-prod','Production qualification/release'], next:'Obtain approved non-production provider configuration, then rerun live integration.'};
document.querySelector('.no-go').textContent = `Production NO-GO · MOCK / REFERENCE only: Connector contracts ${S.suite.pass}/${S.suite.run}; mock PostgreSQL E2E documented; full mock flow passed previously; zero live calls. Commit ${S.snapshot}.`;
document.querySelector('.evidence-note').innerHTML = `<h2>Current facts</h2><ul><li>Commit ${S.snapshot}; mock/local provider environment.</li><li>Connector contract tests: ${S.suite.pass}/${S.suite.run} passed.</li><li>Mock PostgreSQL E2E documented; full mock flow passed previously.</li><li>Yellow live provider gap: approved non-production configuration, endpoints, and credentials absent.</li><li>Red production qualification: live provider, deployment, recovery, load/SLO, RPO/RTO, and approval evidence absent.</li><li><strong>Exactly one next roadmap item:</strong> ${S.next}</li></ul>`;
const items = [
