...
document.querySelector('.no-go').textContent = `${S.decision} · ${S.evidenceLabel}: remediation ${S.commits.remediation}; ${S.remediationStage}; ${S.evidenceScope}. Prior test run ${E.runDate}.`;
document.querySelector('.evidence-note').innerHTML = `<h2>Current facts</h2><ul><li>Remediation ${S.commits.remediation}; ${S.remediationStage}; ${S.evidenceScope}.</li><li>Prior test run (${E.runDate}): Nix full ${E.nixFull.passed}/${E.nixFull.failed}; disposable PostgreSQL ${E.disposablePostgresqlFull.passed}/${E.disposablePostgresqlFull.failed}; targeted P4 ${E.targetedP4.passed}/${E.targetedP4.failed}; labels ${E.labels.passed}/${E.labels.total}.</li><li>Yellow live provider gap: approved non-production configuration, endpoints, and credentials absent.</li><li>Red production qualification: live provider, deployment, recovery, load/SLO, RPO/RTO, and approval evidence absent.</li><li><strong>Exactly one next roadmap item:</strong> ${S.next}</li></ul>`;
const items = [
